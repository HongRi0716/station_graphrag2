...
class AgentChatService:
    ...
    def _parse_websocket_message(
        ...
        try:
            ...
            config_error = agent_config_invalid(
                "agent_message", f"Unexpected error: {str(e)}")
    ...
    async def handle_websocket_agent_chat(self, websocket: WebSocket, user: str, bot_id: str, chat_id: str):
        ...
        if bot_config and bot_config.agent:
            ...
            if bot_config.agent.collections:
                collection_ids = [
                    collection.id for collection in bot_config.agent.collections]
        ...
        try:
            ...
            consumer_task = asyncio.create_task(
                self._consume_messages_from_queue(
                    chat_id, message_id, trace_id, message_queue, websocket)
            )
            ...
            logger.error(
                f"Unexpected error processing agent websocket message: {e}")
            error_response = format_processing_error(
                str(e), agent_message.language or "en-US")
            await websocket.send_text(json.dumps(error_response))
        finally:
            if trace_id:
    ...
    async def register_message_queue(self, language, chat_id, message_id, message_queue):
        ...
        if not trace_id:
            logger.error(
                "Could not get trace_id from current span, event dispatching will fail.")
        else:
    ...
    async def _stream_message_content(
        ...
        for i, chunk in enumerate(chunks):
            ...
            logger.debug(
                f"Sent message chunk {i + 1}/{len(chunks)}: {len(chunk)} chars")
    ...
    async def _consume_messages_from_queue(
        ...
        try:
            ...
            while True:
                ...
                if message is None:
                    logger.debug(
                        "Received end-of-stream signal from message queue")
                    break
                ...
                if isinstance(message, dict) and message.get("type") == "message":
                    ...
                    logger.debug(
                        f"Streamed message content: {message.get('type', 'unknown')}")
                else:
                    # Send other message types normally (start, stop, tool_call_result, etc.)
                    await websocket.send_text(json.dumps(message))
                    logger.debug(
                        f"Sent message to WebSocket: {message.get('type', 'unknown')}")
    ...
    async def _get_agent_session(
        ...
        if not aperag_api_key:
            ...
            logger.info(
                f"No aperag API key found for user {user}, creating a new system key")
            try:
                ...
                logger.info(
                    f"Successfully created new system aperag API key for user {user}")
            except Exception as e:
        ...
        system_prompt = (
            custom_system_prompt if custom_system_prompt else get_agent_system_prompt(
    ...
    async def process_agent_message(
        ...
        try:
            ...
            # Allow time for the message to be processed in listener
        ...
        try:
            ...
            process_task = asyncio.create_task(
                self.process_agent_message(
                    agent_message,
                    user_id,
                    # The 'bot' parameter is unused in the target function, so we pass None for now.
                    None,
                    chat_id,
                    message_id,
                    message_queue,
            ...
            logger.error(
                f"Error during internal agent chat for evaluation: {e}")
            error_response = self._format_exception_to_error_response(
                e, agent_message.language or "en-US")
            return error_response
        finally:
            if trace_id:
                await agent_event_listener.unregister_listener(str(trace_id))
    ...
    async def _save_conversation_history(
        ...
        try:
            ...
            if not history_saved:
                logger.warning(
                    f"Failed to save conversation history for chat: {chat_id}")

        except Exception as e:
            # Don't let history saving errors break the flow
            logger.error(
                f"Error saving conversation history for chat {chat_id}: {e}")
