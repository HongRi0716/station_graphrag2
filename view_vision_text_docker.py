#!/usr/bin/env python3
"""在Docker容器内查看Vision-to-Text完整文本"""
...
def view_vision_text(document_id: str):
    """查看指定文档的完整Vision-to-Text文本"""

    print("=" * 80)
    print("Vision-to-Text完整文本查看")
    print("=" * 80)
    print(f"\n文档ID: {document_id}\n")
    ...
    for session in get_sync_session():
        ...
        if vision_index.index_data:
            try:
                ...
                print(f"从向量存储检索到 {len(points)} 个点\n")
                ...
                if vision_texts:
                    print(f"\n{'='*80}")
                    print(f"Vision-to-Text完整内容 (共 {len(vision_texts)} 个片段)")
                    print("="*80)
                    ...
                    total_chars = sum(item['length'] for item in vision_texts)
                    print(
...
if __name__ == "__main__":
    ...
    try:
        view_vision_text(args.document_id)
    except Exception as e:
        print(f"❌ 错误: {e}")
        import traceback
        traceback.print_exc()
